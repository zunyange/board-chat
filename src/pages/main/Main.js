...
function Main() {
  ...
  const fetchBoards = async (page = 0, size = 5) => {
    try {
      ...
      setData(response.data.data.boards); // 게시판 리스트 추출
      const totalBoards = response.data.data.totalPage; // 전체 게시물 수 : totalPage
      const totalPages = Math.ceil(totalBoards / pageSize); //전체 페이지 계산
